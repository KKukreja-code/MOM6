...
subroutine step_MOM(forces_in, fluxes_in, sfc_state, Time_start, time_int_in, CS, &
  ...
  do n=1,n_max
    ...
    !### Perhaps this should be if (CS%t_dyn_rel_thermo == 0.0)
      ...
      call post_tracer_diagnostics_at_sync(CS%Tracer_reg, h, CS%diag_pre_sync, CS%diag, G, GV, US, CS%t_dyn_rel_diag)
...
subroutine ALE_regridding_and_remapping(CS, G, GV, US, u, v, h, tv, dtdia, Time_end_thermo)
  ...
  if (use_ice_shelf) then
    call ALE_regrid(G, GV, US, h, h_new, dzRegrid, tv, CS%ALE_CSp, CS%frac_shelf_h, PCM_cell, dt=dtdia)
  else
    call ALE_regrid(G, GV, US, h, h_new, dzRegrid, tv, CS%ALE_CSp, PCM_cell=PCM_cell, dt=dtdia)
  endif
