...
use coord_adapt,  only : init_coord_adapt, set_adapt_params, build_adapt_grid, end_coord_adapt
...
subroutine initialize_regridding(CS, G, GV, US, max_depth, param_file, mdl, &
  ...
  if (coordinateMode(coord_mode) == REGRIDDING_ADAPTIVE) then
    ...
    call get_param(param_file, mdl, "ADAPT_SLOPE_CUTOFF", adapt_cutoff, &
         "Slope cutoff between stratified and unstratified regions", units="nondim", default=1e-2)
    call get_param(param_file, mdl, "ADAPT_SMOOTH_MIN", adapt_smooth, &
         "Minimum weight toward smoothing term", units="nondim", default=0.)
...
!------------------------------------------------------------------------------
  ...
  select case ( CS%regridding_scheme )
    ...
    case ( REGRIDDING_ADAPTIVE )
      call build_grid_adaptive(G, GV, G%US, h, tv, CS, dzInterface, dt)
      call calc_h_new_by_dz(CS, G, GV, h, dzInterface, h_new)
...
subroutine build_grid_adaptive(G, GV, US, h, tv, CS, dzInterface, dt)
  ...
  do j = G%jsc-1,G%jec+1
    do i = G%isc-1,G%iec+1
      call check_grid_column(GV%ke, h(i,j,:), dzInterface(i,j,:), 'in build_grid_adaptive')
    enddo
  enddo
...
subroutine set_regrid_params( CS, boundary_extrapolation, min_thickness, old_grid_weight, &
  ...
  if (present(depth_of_time_filter_shallow)) CS%filter_CS%depth_of_time_filter_shallow = &
